...
router.put('/me', auth, async (req, res, next) => {
  try {
    ...
    if (req.body.notificationPrefs && typeof req.body.notificationPrefs === 'object') {
      const prefAllowed = ['taskAssigned', 'taskCompleted', 'deadline', 'chat'];
      const current = (await User.findById(req.user._id)).notificationPrefs || {};
      const merged = { ...current.toObject?.() ?? current };
      for (const k of prefAllowed) if (k in req.body.notificationPrefs) merged[k] = !!req.body.notificationPrefs[k];
